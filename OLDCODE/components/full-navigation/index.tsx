import {
  ...
  searchMovie,
} from "@/features/currentGenreOrCategory/CurrentGenreOrCategory";
...
const FullNavigation: FC<Props> = ({ data, isMenuOpen, setIsMenuOpen }) => {
  ...
  useEffect(() => {}, [genreIdOrCategoryName]);
  return (
    <div
      className={`sidebar z-50 bg-light-background-primary dark:bg-dark-background-secondary text-blue-100 w-64 space-y-6 py-7 px-2 absolute inset-y-0 left-0 transform  md:relative md:translate-x-0 transition duration-200 ease-in-out overflow-auto  ${
        isMenuOpen || width > 768 ? "block " : "hidden"
      }`}
    >
        ...
        {sidebarMenu.map((name, i) => {
          return (
            <Link href="/" key={i} passHref legacyBehavior>
              <Typography
                as="h6"
                className="transition duration-500 ease-in-out cursor-pointer dark:text-white text-sm text-light-2 py-2 group-hover:text-light-3 group-hover:font-semibold hover:text-light-3 dark:hover:text-light-3"
                onClick={() => {
      ...
      <div className="border-t-4 border-dark-background-primary dark:border-light-background-secondary">
        <Typography
          as="h4"
          className="tracking-widest my-6 text-gray-500 uppercase dark:text-white font-title"
        >
          Genres
        </Typography>
          ...
          {data?.genres.map(({ name, id }) => {
            return (
              <Link href="/" passHref key={id} legacyBehavior>
                ...
                >
                  <span className="p-2 text-light  bg-light-accent rounded-xl group-hover:text-light-3 group-hover:bg-orange">
                    <svg
