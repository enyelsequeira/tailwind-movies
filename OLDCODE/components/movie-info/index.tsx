...
const MovieInfo: FC<Props> = ({ data }) => {
  ...
  const { addToFavorites, isAddToFavorite } = useAdd({
    userId: user?.id,
    sessionId,
    movieId: data.id,
    data,
  });
  const { addToWatchlist, isMovieWatchlisted } = useAdd({
    userId: user?.id,
    sessionId,
    movieId: data.id,
    data,
    ...
    return (
      <aside className="px-1 md:px-[10px] py-2  md:col-span-6 lg:col-span-3 h-full md:max-h-full">
        <div className="mt-5  border-red-900  flex flex-col items-center md:items-center">
          <Typography as="h3" className="md:tracking-wider text-center">
            {data.title}
          </Typography>
          <Typography as="h4" className="tracking-wide my-5">
            {" "}
        ...
        <div className="flex flex-between space-x-16  md:space-x-48 ">
          <div>
            <Typography as="h3" className="my-2">
              Rating
            </Typography>
            <Circle
              animate={true}
          ...
          <div className="flex flex-col justify-end space-y-8">
            <Button variant="primary">
              <Typography
                onClick={() => alertNotLoggedIn(addToFavorites, "Favorite")}
              >
                {isAddToFavorite ? "Unfavorite" : "Favorite"}
              </Typography>
              {isAddToFavorite ? <MdFavorite /> : <MdFavoriteBorder />}
            </Button>
            <Button variant="primary">
              <Typography
                onClick={() => alertNotLoggedIn(addToWatchlist, "WatchList")}
              >
                {isMovieWatchlisted ? "Remove WatchList" : "WatchList"}
              </Typography>
              <MdWatchLater />
            </Button>
          </div>
