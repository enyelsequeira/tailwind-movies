    ...
    <div className="relative flex flex-1 max-w-full  mt-2">
      <div className="relative mt-1 w-full">
        ...
        {value && value?.length > 0 && (
          <ul className="absolute mt-2  max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm space-y-2 flex flex-col">
                ...
                {searchedMovies?.results?.map((movie, i) => {
                  return (
                    ...
                    >
                      {match(movie)
                        .with({ media_type: "tv" }, () => (
                          <Link href={`/shows/${movie.id}`}>{movie.name}</Link>
                        ))
